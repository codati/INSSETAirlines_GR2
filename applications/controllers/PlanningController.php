...
class PlanningController extends Zend_Controller_Action
{
    ...
    {
        header('Location: /planning/planifier');
    }

    /**
     * Permet de planifier un vol
     * 
     * @return null
     */
    ...
    /**
     * Permet de modifier le planning d'un vol
     * 
     * @return null
     */
    ...
    /**
     * Recapitulation de la plannification d'un vol avant validation
     * 
     * @return null
     */
    public function recapmodifvolAction() 
    {
        if (Services_verifAcces('Planning')) {
            $this->_helper->actionStack('header', 'index', 'default', array('head' => $this->headStyleScript));
    ...
    public function planifiervolAction($idVol = null)
    {
        if (Services_verifAcces('Planning')) {
            ...
            if ($idVol != null) {
                ...
                header('Location: '.$this->view->baseUrl().'/planning/planifier');
            }
        } else {
            header('Location: /');
        }
    ...
    /**
     * Valide la plannification d'un vol
     * 
     * @return null
     */
            ...
            if ($idVol != null) {
                ...
                if ($infosVol['matriculeAvion'] == null) {
                    ...
                    foreach ($LstClasses as $val) {
                        ...
                        if ($prix != null) {
                            $tableValoir->insertPrixVol($idVol, $val['idClasse'], $prix);
                        }
                    }
                    echo '1';
                }
    ...
    public function lstavionAction($idModele=0, $dateDepart=0, $dateArrivee=0)
    {
        if (Services_verifAcces('Planning')) {
            ...
            if ($idModele == null || $dateDepart == null || $dateArrivee == null) {
                exit;
            } //On vérifie qu'on a bien tout, sinon fin.
    ...
    /**
     * Affiche le calendrier d'un pilote
     * 
     * @return null
     */
    public function calendrierpiloteAction()
    {
        if (Services_verifAcces('Planning')) {
            ...
            if ($idPilote == null || $dateDepart == null || $dateArrivee == null) {
                exit;
            }
            ...
            for ($i=1;$i<=4;$i++) {
                ...
                if (count($lst_dateSQL) > 0) {
                    foreach ($lst_dateSQL as $key => $val) {
                        if ($val['dateHeureDepartEffectiveVol'] == null) {
                            $lst_date[$key]['dep'] = $val['dateHeureDepartPrevueVol'];
                        } else {
                            $lst_date[$key]['dep'] = $val['dateHeureDepartEffectiveVol'];
                        }

                        if ($val['dateHeureArriveeEffectiveVol'] == null) {
                            $lst_date[$key]['arr'] = $val['dateHeureArriveePrevueVol'];
                        } else {
                            $lst_date[$key]['arr'] = $val['dateHeureArriveeEffectiveVol'];
                        }
    ...
    /**
     * Affiche le formulaire pour les prix par classe
     * 
     * @return null
     */
    ...
    public function modifplanningAction()
    {
        if (Services_verifAcces('Planning')) {
            ...
            $this->view->ListeVol = $volsPlanifs;
    ...
    public function finmodifplanifAction()
    {
        if (Services_verifAcces('Planning')) {
            ...
            if ($idVol != null) {
                ...
                // supprime les lignes de "assurer" car elles deviennent obsoletes
                $tableAssurer->deleteVolPilote($idVol, $pilote);
                $tableAssurer->deleteVolPilote($idVol, $copilote);
                // ajoutes les nouvelles lignes avec les nouveaux pilotes
                $tableAssurer->insertPilote($idVol, $pilote);
                $tableAssurer->insertCoPilote($idVol, $copilote);
                ...
                foreach ($LstClasses as $val) {
                    $prix = $this->_getParam('class_'.$val['idClasse'], null);
                    if ($prix != null) {
                        $tableValoir->updatePrixVol($idVol, $val['idClasse'], $prix);                        
                    }
                }
                echo '1';
            }
